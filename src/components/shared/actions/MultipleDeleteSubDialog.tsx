...
export default function MultipleDeleteSubDialog(props: Props) {
  const { actorCommonName, open, handleDialog, itemId, handleDeletedItem, handleMoreClose } = props;
  ...
  const handleDeletion = async (name: string, itemIds: string) => {
    ...
    if (response.ok) {
      ...
      const errorMessage = errorData.message || `Subscription(s) could not be deleted, try again!`;
      ...
      <Dialog open={open} onClose={() => handleDialog(false)}>
        ...
        <DialogActions sx={{ display: "flex", justifyContent: "space-evenly" }}>
          ...
          <StyledButton
            ...
            onClick={() => handleDeletion(actorCommonName, itemId)}
            disableElevation
          >
            Yes, remove
          </StyledButton>
